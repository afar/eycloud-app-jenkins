...
run "cd #{config.latest_release}; sudo bundle exec chef-solo -c #{config.latest_release}/deploy/solo.rb -j /etc/chef-custom/dna.json"

run "cat #{config.latest_release}/deploy/chef-stacktrace.out 2> /dev/null || echo 'Chef run successful'"
